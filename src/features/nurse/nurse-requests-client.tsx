...
import { bookingSourceLabel, canAcceptAssignment, canDeclineAssignment, homeCareStatusLabel, preferredTimeLabel, requestDistanceLabel } from "@/features/nurse/nurse-utils";
...
export function NurseRequestsClient() {
  ...
  const requestsQuery = useQuery({
    queryKey: ["home-care", "requests", "nurse"],
  ...
  const activeRequests = requests.filter((item) => !["care_completed", "patient_confirmed", "cancelled", "unreachable"].includes(item.status));

  return (
    <Section title="Available requests" description="Review assigned offers, confirm the next steps, and open a request when you are ready to continue.">
      ...
      {requestsQuery.isLoading ? (
        <div className="rounded-[24px] border border-slate-200 bg-white px-5 py-10 text-sm text-slate-600">Loading requests...</div>
        ...
        <div className="grid gap-4">
          {activeRequests.map((request) => {
            ...
            return (
              <div key={request.id} className="rounded-[28px] border border-white/70 bg-white p-5 shadow-[0_24px_64px_-42px_rgba(15,23,42,0.45)] sm:p-6">
                <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
                  <div className="min-w-0">
                    ...
                    <p className="mt-2 text-sm leading-6 text-slate-600">{request.service_address_snapshot || "Address details will appear here."}</p>
                  ...
                  <div className="w-full xl:w-[320px]">
                    <div className="rounded-[22px] border border-slate-200 bg-slate-50 p-4">
                      ...
                      <div className="mt-4 flex flex-col gap-3">
                        <Link href={`/nurse/request/${request.id}`} className="inline-flex min-h-11 items-center justify-center rounded-[12px] bg-[var(--primary)] px-4 text-sm font-extrabold text-white transition hover:-translate-y-0.5">
                          Open request
                        </Link>
                        ...
                        {assignment && canDeclineAssignment(assignment, request) ? (
                          <div className="grid gap-2">
                            <input
                              value={declineText}
                              onChange={(event) => setDeclineReason((current) => ({ ...current, [assignment.id]: event.target.value }))}
                            ...
                            <Button
                              variant="secondary"
                              onClick={() => declineMutation.mutate({ id: assignment.id, reason: declineText || "Unavailable for this request right now." })}
