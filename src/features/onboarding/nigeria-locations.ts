// Canonical list of all 36 Nigerian states + the Federal Capital Territory (FCT)
// and their Local Government Areas (774 LGAs total). Used by the onboarding
// Location step to drive the State -> LGA dependent selects.
...
export const NIGERIA_STATES: NigeriaState[] = [
  ...
  },
  {
    ...
    lgas: [
      ...
      "Kaita", "Kankara", "Kankia", "Katsina", "Kurfi", "Kusada", "Mai'Adua", "Malumfashi", "Mani",
  ...
  },
  {
    ...
    lgas: [
      ...
      "Imeko Afon", "Ipokia", "Obafemi Owode", "Odeda", "Odogbolu", "Ogun Waterside", "Remo North",
  ...
  },
  {
    ...
    lgas: [
      ...
      "Ife South", "Ifedayo", "Ifelodun", "Ila", "Ilesa East", "Ilesa West", "Irepodun", "Irewole",
...
export const STATE_NAMES: string[] = NIGERIA_STATES.map((entry) => entry.state);

export function getLgasForState(state: string): string[] {
  return NIGERIA_STATES.find((entry) => entry.state === state)?.lgas ?? [];
}
